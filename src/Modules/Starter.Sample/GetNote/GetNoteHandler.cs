...
{
    public async Task<Result<(Guid Id, string Title, string Body, DateTimeOffset CreatedAt, DateTimeOffset UpdatedAt)>>
        HandleAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        return (note.Id, note.Title, note.Body, note.CreatedAt, note.UpdatedAt);
    }
}
